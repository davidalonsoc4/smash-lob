...
assert(detailPairingPanel.includes('pointer-events-none absolute left-1/2 top-1/2 z-20') && detailPairingPanel.includes('>\n            VS\n          </span>') && detailPairingPanel.includes('flex h-8 w-8 items-center justify-center rounded-full bg-neutral-950 text-[10px] font-black uppercase tracking-wide text-white shadow-sm'), "El panel de detalle debe recuperar un VS flotante sin robar ancho a las parejas")
assert(detailPairingPanel.includes('className={`min-w-0 ${alignment === "right" ? "text-right" : "text-left"}`}') && detailPairingPanel.includes('rounded-lg bg-neutral-50 px-2 py-2'), "Las parejas deben evitar paneles anidados pesados y permitir alineacion propia por columna")
