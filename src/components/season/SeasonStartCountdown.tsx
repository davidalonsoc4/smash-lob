...
function CountdownValue({ days, hours, minutes, seconds, hero = false }: { days: number; hours: number; minutes: number; seconds: number; hero?: boolean }) {
  ...
  const units = [[days, "DÍAS"], [hours, "HORAS"], [minutes, "MIN"], [seconds, "SEG"]] as const
...
export function SeasonStartCountdown({ scheduledStartAt, compact = false, hero = false }: Props) {
  ...
  const formattedStart = formatScheduledSeasonStart(scheduledStartAt)
  ...
  if (compact) return <div className="rounded-xl bg-neutral-100 px-3 py-2.5"><div className="flex items-center justify-between gap-3"><div className="min-w-0"><p className="type-caption font-black uppercase tracking-[0.14em] text-neutral-500">{countdown.isDue ? "Preparando inicio" : "La temporada comienza en"}</p><p className="mt-1 truncate text-xs font-semibold text-neutral-500">{formattedStart}</p></div>{countdown.isDue ? <span className="rounded-full bg-amber-100 px-2.5 py-1 type-caption font-black text-amber-900">Activación automática</span> : <CountdownValue {...countdown} />}</div></div>
  ...
  return <section data-season-start-countdown={hero ? "hero" : "waiting"} className={hero ? "flex min-h-[calc(100dvh-13rem)] flex-col items-center justify-center rounded-[2rem] bg-neutral-950 px-4 py-8 text-white shadow-[0_18px_50px_rgba(15,23,42,0.18)]" : "px-2 py-1 text-center text-neutral-950"}>
    <p className={`${hero ? "text-sm text-neutral-300" : "type-caption text-neutral-500"} font-black uppercase tracking-[0.18em]`}>{countdown.isDue ? "PREPARANDO INICIO" : "LA TEMPORADA COMIENZA EN"}</p>
    {countdown.isDue ? <p className={`${hero ? "my-10 text-4xl" : "my-3 text-xl"} font-black`}>Activación automática</p> : <div className={hero ? "my-8 w-full" : "my-2"}><CountdownValue {...countdown} hero={hero} /></div>}
