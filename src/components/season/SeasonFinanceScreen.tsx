...
const dateLabel = (value: string, locale: Locale) => { const date = new Date(value); return Number.isNaN(date.getTime()) ? "" : new Intl.DateTimeFormat(getIntlLocale(locale), { day: "2-digit", month: "short", year: "numeric", timeZone: "Europe/Madrid" }).format(date) }
...
export function SeasonFinanceScreen() {
  ...
  async function createExpense() {
    ...
    try { await mutateExpense("POST", { title: title.trim(), amount: numericAmount }); setTitle(""); setAmount("") } catch { setError(tx("No se ha podido registrar el gasto.")) } finally { setBusy(false) }
  }
  async function deleteExpense(expenseId: string) {
    if (!canEdit || busy || !window.confirm(tx("¿Eliminar este gasto de la temporada?"))) return
    setBusy(true); setError(null)
  ...
  if (!roundSettings.registrationFee.enabled || roundSettings.registrationFee.amount <= 0) return <div className="space-y-4"><header className="app-page-header"><BackButton fallbackHref="/admin/season" label={tx("Volver")} /><h1 className="type-page-title">{tx("Economía de temporada")}</h1></header><AppCard><p className="font-black">{tx("Sin inscripción")}</p><p className="mt-1 text-sm font-semibold text-neutral-600">{tx("Activa una inscripción en Ajustes de temporada para registrar ingresos y gastos.")}</p></AppCard></div>

  return <div className="space-y-4">
    <header className="app-page-header"><BackButton fallbackHref="/admin/season#inscripcion" label={tx("Volver")} /><h1 className="type-page-title">{tx("Economía de temporada")}</h1><p className="mt-0.5 type-caption font-bold text-neutral-500">{activeSeason.name} {tx("· Inscripción")}{" "}{formatMoney(roundSettings.registrationFee.amount)}{tx("/jugador")}</p></header>
    <div className="grid grid-cols-2 gap-2" data-season-finance-summary><FinanceStat label={tx("Ingresado")} value={formatMoney(summary.collected)} helper={tx(`${summary.paidCount} pagos reales`)} /><FinanceStat label={tx("Pendiente")} value={formatMoney(summary.pending)} helper={tx(`${summary.pendingCount} jugadores`)} /><FinanceStat label={tx("Gastado")} value={formatMoney(summary.spent)} helper={tx(`${expenses.length} gastos`)} /><FinanceStat label={tx("Disponible")} value={formatMoney(summary.available)} helper={tx(`${formatMoney(summary.availablePerPlayer)} POR PERSONA`)} negative={summary.available < 0} /></div>
    {canEdit ? <AppCard data-season-expense-form><p className="font-black">{tx("Registrar gasto")}</p><div className="mt-3 grid gap-3"><label><span className="type-caption font-black uppercase tracking-wide text-neutral-500">{tx("Título")}</span><input value={title} onChange={(event) => setTitle(event.target.value)} maxLength={120} placeholder={tx("Bolas jornada de apertura")} className="mt-1.5 w-full rounded-2xl border border-neutral-200 bg-white px-3 py-2.5 text-sm font-semibold outline-none" /></label><label><span className="type-caption font-black uppercase tracking-wide text-neutral-500">{tx("Importe")}</span><div className="mt-1.5 flex items-center rounded-2xl border border-neutral-200 bg-white px-3 py-2.5"><input type="number" min="0.01" step="0.01" value={amount} onChange={(event) => setAmount(event.target.value)} placeholder="0,00" className="min-w-0 flex-1 bg-transparent text-sm font-black outline-none" /><span className="font-black text-neutral-500">€</span></div></label></div><button type="button" onClick={() => void createExpense()} disabled={busy || !valid} className="mt-3 flex w-full items-center justify-center rounded-2xl bg-neutral-950 px-4 py-3 text-center text-sm font-black text-white disabled:bg-neutral-200 disabled:text-neutral-500">{busy ? tx("Guardando...") : tx("Añadir gasto")}</button>{error ? <p className="mt-2 text-center text-xs font-semibold text-red-600">{tx(error)}</p> : null}</AppCard> : null}
    <AppCard><div className="flex items-center justify-between gap-3"><p className="font-black">{tx("Gastos registrados")}</p><span className="type-caption font-black text-neutral-500">{formatMoney(summary.spent)}</span></div>{expenses.length ? <div className="mt-3 divide-y divide-neutral-100">{expenses.map((expense) => <div key={expense.id} className="flex items-center gap-3 py-2.5 first:pt-0 last:pb-0"><div className="min-w-0 flex-1"><p className="truncate text-sm font-black text-neutral-950">{expense.title}</p><p className="mt-0.5 type-caption font-semibold text-neutral-500">{dateLabel(expense.createdAt, locale)}</p></div><p className="shrink-0 text-sm font-black text-neutral-950">{formatMoney(expense.amount)}</p>{canEdit ? <button type="button" aria-label={tx(`Eliminar gasto ${expense.title}`)} onClick={() => void deleteExpense(expense.id)} disabled={busy} className="grid h-8 w-8 shrink-0 place-items-center rounded-full text-lg font-black text-neutral-400 hover:bg-red-50 hover:text-red-600 disabled:opacity-30">×</button> : null}</div>)}</div> : <p className="mt-3 rounded-2xl bg-neutral-100 px-3 py-3 text-sm font-semibold text-neutral-600">{tx("Todavía no hay gastos registrados.")}</p>}</AppCard>
    <button type="button" disabled data-season-finance-report className="flex w-full items-center justify-center rounded-2xl border border-neutral-200 bg-white px-4 py-3 text-center text-sm font-black text-neutral-500 shadow-sm disabled:cursor-not-allowed disabled:opacity-70">{tx("Generar informe de transparencia · Próximamente")}</button>
  </div>
}
