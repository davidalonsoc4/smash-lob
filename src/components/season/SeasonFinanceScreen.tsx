...
export function SeasonFinanceScreen() {
  ...
  return <div className="space-y-4">
    ...
    <div className="grid grid-cols-2 gap-2" data-season-finance-summary><FinanceStat label="Ingresado" value={formatMoney(summary.collected)} helper={`${summary.paidCount} pagos reales`} /><FinanceStat label="Pendiente" value={formatMoney(summary.pending)} helper={`${summary.pendingCount} jugadores`} /><FinanceStat label="Gastado" value={formatMoney(summary.spent)} helper={`${expenses.length} gastos`} /><FinanceStat label="Disponible" value={formatMoney(summary.available)} helper="Ingresado − gastado" negative={summary.available < 0} /></div>
