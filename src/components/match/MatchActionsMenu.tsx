...
export function MatchActionsTrigger({ match, isAdmin, canReportIncident, canManageSubstitutions, chatHref, menuOpen, onMenuOpenChange, onSelectPanel }: MatchActionsTriggerProps) {
  ...
  return (
    <div className="fixed z-40 flex flex-col items-end gap-2" style={{ right: "max(14px, calc((100vw - 448px) / 2 + 14px))", bottom: "calc(84px + env(safe-area-inset-bottom, 0px))" }}>
      {chatHref ? <Link href={chatHref} aria-label="Abrir chat del partido" title="Chat del partido" className="app-floating-control grid h-10 w-10 place-items-center rounded-full border border-neutral-200 bg-white/95 text-neutral-700 shadow-lg backdrop-blur transition active:scale-95 active:bg-neutral-100"><ChatIcon /></Link> : null}
