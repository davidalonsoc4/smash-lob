          ...
          return (
            <div key={tour.key} className="rounded-2xl bg-neutral-100 p-3">
              <div className="flex items-start justify-between gap-3">
                ...
                <span className={`shrink-0 rounded-full px-2 py-1 text-[9px] font-black uppercase tracking-wide ${completed ? "bg-emerald-100 text-emerald-700" : "bg-white text-neutral-500"}`}>
                  {completed ? copy.completed : copy.pending}
                </span>
              </div>
              <Link
                href={`${tour.route}?tour=${encodeURIComponent(tour.key)}`}
                className="mt-2 inline-flex rounded-xl bg-white px-3 py-2 text-[11px] font-black text-neutral-800 shadow-sm"
              >
                {copy.openScreen}
              </Link>
            </div>
