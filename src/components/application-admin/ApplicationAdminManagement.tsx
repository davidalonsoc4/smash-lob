...
function ApplicationUsersView() {
  ...
  return (
    <div className="compact-page space-y-3">
      <header className="app-page-header">
        <BackButton fallbackHref="/application-admin" label="Volver" />
        <h1 className="type-page-title font-black tracking-tight">Gestión de usuarios</h1>
        <p className="mt-0.5 text-xs font-black uppercase tracking-[0.16em] text-red-600">
          Superusuario
        </p>
      </header>
<AppCard>
...
function ApplicationLocationsView() {
  ...
  useEffect(() => {
    ...
    fetchManagedLocations()
      ...
      .catch(() => {
        if (!cancelled) setError("No se han podido cargar las ubicaciones.")
      })
      .finally(() => {
        if (!cancelled) setLoading(false)
      })
...
export default function ApplicationAdminManagement({ mode }: { mode: "users" | "locations" }) { return mode === "locations" ? <ApplicationLocationsView /> : <ApplicationUsersView /> }
