...
const TRIM_WIDTH_MM = 90
const TRIM_HEIGHT_MM = 18
const BLEED_MM = 2
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .accent-top, .accent-bottom { position: absolute; left: 0; right: 0; height: .65mm; background: ${accent}; }
  .accent-top { top: 0; }
  .accent-bottom { bottom: 0; opacity: .72; }
  .content { position: absolute; left: ${BLEED_MM}mm; top: ${BLEED_MM}mm; width: ${TRIM_WIDTH_MM}mm; height: ${TRIM_HEIGHT_MM}mm; display: grid; grid-template-columns: 18mm .4mm 1fr; align-items: center; gap: 4mm; padding: 2.2mm 5mm 2.2mm 3mm; color: white; }
  .logo { height: 12mm; display: flex; align-items: center; justify-content: center; }
  .logo img { display: block; max-width: 17mm; max-height: 11mm; object-fit: contain; filter: drop-shadow(0 1.2mm 2mm rgba(0,0,0,.35)); }
  .fallback { font-weight: 900; font-size: 5mm; letter-spacing: .8mm; }
  .divider { width: .4mm; height: 9mm; background: ${accent}; border-radius: 999px; }
  .name { min-width: 0; text-align: center; font-size: 4mm; line-height: 1; font-weight: 900; text-transform: uppercase; letter-spacing: .28mm; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  ...
  return (
    <div className="mx-auto mt-4 w-full max-w-[430px] rounded-xl border border-neutral-200 bg-white p-3 shadow-sm">
      <div className="mb-3 flex items-center justify-between gap-3">
        ...
        <span className="shrink-0 rounded-full bg-neutral-100 px-2.5 py-1 text-[0.5625rem] font-black uppercase tracking-[.08em] text-neutral-700">90 × 18 mm</span>
      </div>

      <div className="rounded-[18px] bg-[#f4f1ea] p-4 ring-1 ring-neutral-200">
        <div
          className="relative mx-auto w-full max-w-[380px] overflow-hidden rounded-[10px] border border-black/40 text-white shadow-[0_16px_30px_rgba(0,0,0,.28)]"
          style={{
        ...
        >
          ...
          <div className="relative z-10 grid h-full grid-cols-[70px_2px_minmax(0,1fr)] items-center gap-4 px-5">
            <div className="flex min-w-0 items-center justify-center">
              {leagueLogoUrl ? (
                <Image unoptimized src={leagueLogoUrl} alt={leagueName} width={90} height={56} className="h-auto max-h-[48px] w-auto max-w-[68px] object-contain drop-shadow-[0_7px_14px_rgba(0,0,0,.4)]" />
              ) : (
                <span className="text-xl font-black tracking-[.16em] text-white">{initials(leagueName)}</span>
              )}
            </div>
            <span className="h-[46px] w-[2px] rounded-full" style={{ backgroundColor: accent }} />
            <p className="truncate text-center text-[0.72rem] font-black uppercase tracking-[.08em] text-white">{leagueName}</p>
          </div>
        </div>
      </div>

      <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-[0.625rem] font-semibold leading-4 text-neutral-500">Corte 90 × 18 mm · sangrado 2 mm · impresión 94 × 22 mm · vinilo adhesivo mate.</p>
