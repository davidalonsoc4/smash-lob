...
type ExportKind =
  ...
  | "ranking"
  | "summary"
type BusyAction = "preview" | "share" | "download" | null
...
export function SeasonShareExportsCard({
  ...
  async function openPreview(kind: ExportKind) {
    ...
    try {
      ...
      showActionFeedback({ tone: "error", message: tx("No se ha podido generar la imagen.") })
    } finally {
  ...
  async function sharePreview() {
    if (busyAction || !previewBlob || !previewKind) return
    setBusyAction("share")

    try {
      const filename = getFilename(previewKind)
      ...
      if (navigator.share && navigator.canShare?.({ files: [file] })) {
        ...
        if (previewKind === "summary") downloadSeasonSummaryImage(previewBlob, filename)
        else downloadSeasonExportImage(previewBlob, filename)
        showActionFeedback({
          tone: "info",
      ...
      if (!(error instanceof DOMException && error.name === "AbortError")) {
        showActionFeedback({ tone: "error", message: tx("No se ha podido compartir la imagen.") })
      }
    } finally {
      setBusyAction(null)
    }
  ...
  async function downloadPreview() {
    ...
    try {
      ...
      if (previewKind === "summary") {
        ...
        downloadSeasonExportImage(previewBlob, filename)
        showActionFeedback({ tone: "success", message: tx("Imagen guardada correctamente.") })
      }
    } catch {
      showActionFeedback({ tone: "error", message: tx("No se ha podido descargar la imagen.") })
    } finally {
      setBusyAction(null)
    }
  }

  const actionDisabled = Boolean(activeOption?.disabled || busyAction)
  const previewOption = previewKind
    ? exportOptions.find((option) => option.kind === previewKind)
    : activeOption
  ...
  return (
    ...
    <AppCard className="space-y-4 border-neutral-200 bg-white shadow-sm">
        ...
        <div className="grid grid-cols-2 gap-2">
          {exportOptions.map((option) => (
            <ExportSelector
              ...
              onSelect={() => setActiveKind(option.kind)}
            />
          ))}
        </div>
      </div>
      ...
      {activeOption ? (
        <div className="space-y-3">
          ...
          <button
            type="button"
            data-season-export-preview
            onClick={() => void openPreview(resolvedActiveKind)}
    ...
    <GeneratedImagePreviewModal
      open={previewOpen}
      title={previewOption?.title ?? tx("Imagen de la temporada")}
