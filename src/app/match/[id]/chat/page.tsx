...
const proposalDate = (value: string, locale: Locale) => new Intl.DateTimeFormat(getIntlLocale(locale), { day: "2-digit", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" }).format(new Date(value))
const reservationSummaryDate = (value: string, locale: Locale) => new Intl.DateTimeFormat(getIntlLocale(locale), { day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit", hour12: false }).format(new Date(value))
const proposalVoteDate = (value: string, locale: Locale) => { const formatted = new Intl.DateTimeFormat(getIntlLocale(locale), { weekday: "long", day: "numeric", month: "long", year: "numeric", hour: "2-digit", minute: "2-digit", hour12: false }).format(new Date(value)); return formatted.charAt(0).toLocaleUpperCase(getIntlLocale(locale)) + formatted.slice(1) }
...
const calendarWeekday = (value: string, locale: Locale) => new Intl.DateTimeFormat(getIntlLocale(locale), { weekday: "short" }).format(parseLocalDate(value)).replace(".", "").toLocaleUpperCase(getIntlLocale(locale))
...
export default function MatchChatPage() {
  ...
  function proposalControls(message: Message, optionKey: string) {
    ...
    return <div className="flex shrink-0 items-center gap-1.5"><button type="button" aria-pressed={mine === "available"} onClick={(event) => { event.stopPropagation(); void respond(message.id, optionKey, "available") }} aria-label={tx(`${mine === "available" ? "Quitar mi voto favorable" : "Me viene bien"} · ${yes} votos`)} title={tx(mine === "available" ? "Quitar voto" : "Me viene bien")} className={`inline-flex h-10 min-w-10 items-center justify-center gap-1 rounded-full border px-2 transition active:scale-95 ${mine === "available" ? "border-emerald-600 bg-emerald-600 text-white" : idleVoteClass}`}><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5"><path d="m5 12 4 4L19 6" /></svg><span className="type-caption font-black">{yes}</span></button><button type="button" aria-pressed={mine === "unavailable"} onClick={(event) => { event.stopPropagation(); void respond(message.id, optionKey, "unavailable") }} aria-label={tx(`${mine === "unavailable" ? "Quitar mi voto negativo" : "No puedo"} · ${no} votos`)} title={tx(mine === "unavailable" ? "Quitar voto" : "No puedo")} className={`inline-flex h-10 min-w-10 items-center justify-center gap-1 rounded-full border px-2 transition active:scale-95 ${mine === "unavailable" ? "border-red-600 bg-red-600 text-white" : idleVoteClass}`}><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" className="h-5 w-5"><path d="m6 6 12 12M18 6 6 18" /></svg><span className="type-caption font-black">{no}</span></button></div>
  }
  ...
  return (
    <MatchChatFrame viewportRef={viewportRef} backHref={`/match/${id}`} title={matchRound ? tx(`Chat · Jornada ${matchRound}`) : tx("Chat del partido")} titleHref={`/match/${id}`}>
        ...
        <div>
          {messages.map((message, index) => {
            ...
            if (message.kind === "text" && payload.systemType === "reservation_agreement_invalidated") {
              return <div key={messageKey} className={`flex justify-center ${rowSpacing} ${enterClass}`}><div className="w-[96%] rounded-xl border border-amber-200 bg-amber-50 px-2.5 py-2 text-center"><p className="type-caption font-black text-amber-800">{tx("Fecha/hora acordada descartada")}</p><p className="mt-0.5 type-caption font-semibold text-amber-700">{tx("Hay que hacer una nueva propuesta de fecha y hora.")}</p></div></div>
            }
            if (message.kind === "text" && payload.systemType === "reservation_confirmed") {
              ...
              return <div key={messageKey} className={`flex justify-center ${rowSpacing} ${enterClass}`}><div className="w-[96%] rounded-2xl border border-emerald-200 bg-emerald-50 p-2.5 text-neutral-950 shadow-sm"><p className="type-caption font-black uppercase tracking-[0.14em] text-emerald-700">{tx("Partido programado")}</p><p className="mt-1 text-sm font-black">{scheduledAt ? proposalDate(scheduledAt, locale) : String(payload.dateLabel ?? "")}</p><p className="mt-0.5 text-xs font-bold text-neutral-600">{locationText ?? tx("Pista reservada")}</p>{match && scheduledAt ? <div className="mt-2"><AddToCalendarButton leagueName={activeLeague.name} seasonName={activeSeason.name} round={matchRound ?? match.round} teamA={match.teamA} teamB={match.teamB} players={players} scheduledAt={scheduledAt} location={typeof payload.location === "string" ? payload.location : locationText} /></div> : null}</div></div>
            }
            if (message.kind !== "text") {
              ...
              return <div key={messageKey} {...replyGestureProps} className={`flex items-start gap-1 ${mine ? "justify-end" : "justify-start"} ${rowSpacing} ${enterClass}`}>{avatarSlot}<div className={`w-[90%] rounded-2xl p-2.5 shadow-sm ${mine ? "border border-transparent bg-clip-padding bg-neutral-950" : "border border-neutral-200 bg-white text-neutral-950"} ${!previousSameSender ? mine ? "rounded-tr-md" : "rounded-tl-md" : ""}`}>{!mine && !previousSameSender ? senderHref ? <Link href={senderHref} onClick={(event) => event.stopPropagation()} className={`block w-fit max-w-full truncate type-caption font-black underline-offset-2 active:underline ${senderColor}`}>{message.sender_display_name}</Link> : <p className={`truncate type-caption font-black ${senderColor}`}>{message.sender_display_name}</p> : null}<button type="button" aria-expanded={proposalExpanded} onClick={(event) => { event.stopPropagation(); setExpandedProposalOptionKey(null); setExpandedProposalId((current) => current === message.id ? null : message.id) }} className={`flex w-full items-start justify-between gap-3 text-left ${mine ? "bg-neutral-950 text-white" : "text-neutral-950"}`}><div className="min-w-0"><p className="text-sm font-black">{message.kind === "date_proposal" ? tx("Propuesta de fecha") : tx("Propuesta de ubicación")}</p><p className={`type-caption font-semibold ${mine ? "text-neutral-300" : "text-neutral-400"}`}>{tx("Toca para ver quién ha votado")}</p></div><span className="shrink-0"><span className={`inline-flex whitespace-nowrap leading-none ${mine ? "text-neutral-300" : "text-neutral-400"}`}><span className="origin-right scale-90 type-caption">{new Date(message.created_at).toLocaleTimeString(getIntlLocale(locale), { hour: "2-digit", minute: "2-digit" })}{mine ? <MatchChatMessageReceipt message={message} me={me} participants={participants} pending={message.id.startsWith("optimistic-")} /> : null}</span></span></span></button>{message.kind === "date_proposal" ? <div className="mt-1.5 space-y-1.5">{(Array.isArray(payload.options) ? payload.options : []).map((raw) => { const option = record(raw); const key = String(option.key ?? ""); const startsAt = String(option.startsAt ?? ""); const invalidated = option.invalidated === true; const detailKey = `${message.id}:${key}`, optionExpanded = proposalExpanded || expandedProposalOptionKey === detailKey; return key && startsAt ? <div key={key} className={`flex items-start gap-2 rounded-xl px-2.5 py-1.5 text-neutral-950 ${invalidated ? "bg-neutral-100 opacity-70" : "bg-neutral-50"}`}><div className="min-w-0 flex-1"><button type="button" aria-expanded={optionExpanded} onClick={(event) => { event.stopPropagation(); setExpandedProposalOptionKey((current) => current === detailKey ? null : detailKey) }} className="block w-full text-left"><span className={`block text-sm font-bold ${invalidated ? "line-through text-neutral-500" : ""}`}>{proposalVoteDate(startsAt, locale)}</span>{invalidated ? <span className="mt-0.5 inline-flex rounded-full bg-neutral-200 px-1.5 py-px type-caption font-black uppercase tracking-wide text-neutral-600">{tx("Descartada")}</span> : optionHasAgreement(message, key) ? <span className="mt-0.5 inline-flex rounded-full bg-emerald-100 px-1.5 py-px type-caption font-black uppercase tracking-wide text-emerald-700">{tx("Acuerdo 4/4")}</span> : null}</button>{optionExpanded ? proposalVoteDetailRows(message, key) : null}</div>{invalidated ? <span className="shrink-0 type-caption font-black text-neutral-500">{tx("Nueva propuesta")}</span> : proposalControls(message, key)}</div> : null })}</div> : <div className="mt-1.5 flex items-start gap-2 rounded-xl bg-neutral-50 px-2.5 py-1.5 text-neutral-950"><div className="min-w-0 flex-1"><button type="button" aria-expanded={locationExpanded} onClick={(event) => { event.stopPropagation(); setExpandedProposalOptionKey((current) => current === locationDetailKey ? null : locationDetailKey) }} className={`flex min-h-10 w-full flex-col text-left ${locationAgreed ? "items-start justify-start" : "justify-center"}`}><span className="block text-sm font-bold">{String(payload.name ?? tx("Ubicación propuesta"))}</span>{locationAgreed ? <span className="mt-0.5 inline-flex rounded-full bg-emerald-100 px-1.5 py-px type-caption font-black uppercase tracking-wide text-emerald-700">{tx("Acuerdo 4/4")}</span> : null}</button>{locationExpanded ? proposalVoteDetailRows(message, locationKey) : null}</div>{proposalControls(message, locationKey)}</div>}</div></div>
            }
            const senderHref = participantProfileHref(sender)
      ...
      {error ? <p className="mx-3 mt-2 shrink-0 rounded-xl bg-red-50 px-3 py-2 type-caption font-bold text-red-700">{tx(error)}</p> : null}
      {effectiveReadOnly ? <MatchChatReadOnlyBar>{playerSeasonReadOnly ? tx("Temporada pendiente · Chat en modo lectura") : tx("Partido finalizado · Chat en modo lectura")}</MatchChatReadOnlyBar> : <>
        ...
        {actionMode && !matchFinished ? <div className="mx-3 mt-2 shrink-0 rounded-2xl border border-neutral-200 bg-white p-3 shadow-lg">
          {actionMode === "menu" ? <div><p className="mb-2 type-caption font-black uppercase tracking-[0.14em] text-neutral-400">{tx("Adjuntar al chat")}</p><div className="grid grid-cols-2 gap-2">
            <button type="button" onClick={() => openProposalMode("date")} className="group flex min-h-20 items-center gap-3 rounded-2xl border border-neutral-200 bg-stone-50 px-3 py-3 text-left transition active:scale-[0.98]"><span className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white text-neutral-950 shadow-sm"><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5"><path d="M7 2v3M17 2v3M3.5 9h17" /><rect x="3.5" y="4" width="17" height="17" rx="3" /></svg></span><span className="min-w-0"><span className="block text-sm font-black">{tx("Fecha")}</span><span className="mt-0.5 block type-caption font-bold leading-tight text-neutral-500">{tx("Propón horarios")}</span></span></button>
            <button type="button" onClick={() => openProposalMode("location")} className="group flex min-h-20 items-center gap-3 rounded-2xl border border-neutral-200 bg-stone-50 px-3 py-3 text-left transition active:scale-[0.98]"><span className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-white text-neutral-950 shadow-sm"><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5"><path d="M20 10c0 5-8 12-8 12S4 15 4 10a8 8 0 1 1 16 0Z" /><circle cx="12" cy="10" r="2.5" /></svg></span><span className="min-w-0"><span className="block text-sm font-black">{tx("Ubicación")}</span><span className="mt-0.5 block type-caption font-bold leading-tight text-neutral-500">{tx("Propón una pista")}</span></span></button>
          </div></div> : null}
          {actionMode === "date" ? <div>
            <div className="flex items-center gap-2"><span className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-neutral-950 text-white"><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round" className="h-4 w-4"><path d="M7 2v3M17 2v3M3.5 9h17" /><rect x="3.5" y="4" width="17" height="17" rx="3" /></svg></span><div className="min-w-0 flex-1"><p className="text-sm font-black">{tx("Proponer fecha")}</p><p className="type-caption font-semibold text-neutral-500">{tx("Selecciona un día de las próximas dos semanas")}</p></div><button type="button" onClick={() => { setActionMode(null); setDateDraft(""); setDateOptions([]) }} className="flex items-center justify-center rounded-full bg-neutral-100 px-2.5 py-1 text-center type-caption font-black text-neutral-600">{tx("Cerrar")}</button></div>
            <div className="mt-3 grid grid-cols-7 gap-1">{proposalCalendarDays.map((day) => { const selectedCount = dateOptions.filter((item) => localDateValue(new Date(item)) === day).length; return <button key={day} type="button" onClick={() => { dateDraftTouchedRef.current = true; const recommendedTime = availabilityRecommendations.find((item) => item.date === day)?.start; setDateDraft(`${day}T${recommendedTime ?? selectedProposalTime}`) }} className={`relative flex min-h-12 flex-col items-center justify-center rounded-xl border px-1 py-1.5 text-center transition active:scale-[0.98] ${selectedProposalDate === day ? "border-neutral-950 bg-neutral-950 text-white" : "border-neutral-200 bg-stone-50 text-neutral-700"}`}><span className="type-caption font-black">{calendarWeekday(day, locale)}</span><span className="text-sm font-black leading-none">{calendarDayNumber(day)}</span>{selectedCount ? <span className={`absolute -right-1 -top-1 inline-flex h-4 min-w-4 items-center justify-center rounded-full px-1 type-caption font-black leading-none ${selectedProposalDate === day ? "bg-white text-neutral-950" : "bg-neutral-950 text-white"}`}>{selectedCount}</span> : null}</button> })}</div>
            {selectedDayRecommendedTimes.length ? <div className="mt-3"><p className="mb-1 type-caption font-black uppercase tracking-wide text-neutral-400">{tx("Horarios compatibles")}</p><div className="flex flex-wrap gap-1.5">{selectedDayRecommendedTimes.map((time) => { const iso = new Date(`${selectedProposalDate}T${time}`).toISOString(); const chosen = dateOptions.includes(iso); return <button key={time} type="button" aria-pressed={chosen} onClick={() => toggleDateOption(selectedProposalDate, time)} className={`rounded-full border px-2.5 py-1 type-caption font-black transition active:scale-[0.97] ${chosen ? "border-neutral-950 bg-neutral-950 text-white" : "border-neutral-200 bg-white text-neutral-600"}`}>{chosen ? "✓ " : ""}{time}</button> })}</div></div> : null}
            <div className="mt-3"><label className="block"><span className="mb-1 block type-caption font-black uppercase tracking-wide text-neutral-500">{tx("Otra hora")}</span><input type="time" value={selectedProposalTime} onChange={(event) => selectManualDateOption(event.target.value)} className={`h-10 w-full rounded-xl border px-3 text-sm font-bold outline-none transition ${selectedProposalTimeIsChosen ? "border-neutral-950 bg-neutral-950 text-white" : "border-neutral-200 bg-stone-50 focus:border-neutral-400"}`} /></label></div>
            {availabilityConfigured ? <p className="mt-2 type-caption font-semibold text-neutral-500">{tx("Toca uno o varios horarios compatibles. También puedes elegir cualquier otra hora.")}</p> : null}
            {dateOptions.length ? <div className="mt-2 flex flex-wrap gap-1">{dateOptions.map((item) => { const date = new Date(item); const label = `${calendarWeekday(localDateValue(date), locale)} ${date.getDate()} · ${localTimeValue(date)}`; return <button key={item} type="button" onClick={() => removeDateOption(item)} aria-label={tx(`Quitar ${label}`)} className="inline-flex items-center gap-1 rounded-full bg-neutral-100 px-2 py-1 type-caption font-black text-neutral-600">{label}<span aria-hidden="true">×</span></button> })}</div> : null}
            <div className="mt-2 flex items-center justify-between gap-2"><p className="type-caption font-black text-neutral-700">{dateOptions.length ? tx(`${dateOptions.length} fecha${dateOptions.length === 1 ? "" : "s"} seleccionada${dateOptions.length === 1 ? "" : "s"}`) : tx("Selecciona hasta 4 fechas")}</p>{dateOptions.length >= 4 ? <span className="type-caption font-bold text-neutral-400">{tx("Máximo 4")}</span> : null}</div>
            <button type="button" disabled={!dateOptions.length} onClick={() => void sendDates()} className="mt-3 flex w-full items-center justify-center rounded-xl bg-neutral-950 px-3 py-2.5 text-center text-sm font-black text-white disabled:opacity-40">{dateOptions.length === 1 ? tx("Proponer esta fecha") : tx(`Proponer ${dateOptions.length} fechas`)}</button>
          </div> : null}
          {actionMode === "location" ? <div>
            <div className="flex items-center gap-2"><span className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-neutral-950 text-white"><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round" className="h-4 w-4"><path d="M20 10c0 5-8 12-8 12S4 15 4 10a8 8 0 1 1 16 0Z" /><circle cx="12" cy="10" r="2.5" /></svg></span><p className="flex-1 text-sm font-black">{tx("Proponer ubicación")}</p><button type="button" onClick={() => { setActionMode(null); setLocationDraft("") }} className="flex items-center justify-center rounded-full bg-neutral-100 px-2.5 py-1 text-center type-caption font-black text-neutral-600">{tx("Cerrar")}</button></div>
            ...
            <div className="mt-2"><input value={activeLeague.locations.some((item) => item.id === locationDraft) ? "" : locationDraft} onChange={(event) => setLocationDraft(event.target.value)} placeholder={tx("Otra ubicación…")} className="w-full rounded-xl border border-neutral-200 bg-stone-50 px-3 py-2 text-sm font-bold outline-none placeholder:text-neutral-400 focus:border-neutral-400" /></div>
            <button type="button" disabled={!locationDraft.trim()} onClick={() => void sendLocation()} className="mt-3 flex w-full items-center justify-center rounded-xl bg-neutral-950 px-3 py-2.5 text-center text-sm font-black text-white disabled:opacity-40">{tx("Enviar propuesta")}</button>
          </div> : null}
        </div> : null}
        {matchFinished && !effectiveReadOnly ? <MatchChatWriteWindowBanner writeUntil={writeUntil} /> : null}
        {replyingTo ? <div className="flex shrink-0 items-center gap-2 border-t border-neutral-200 bg-white px-3 py-1.5"><div className="min-w-0 flex-1 border-l-2 border-indigo-400 pl-2"><p className="truncate type-caption font-black text-indigo-600">{tx("Responder a")} {replyingHref ? <Link href={replyingHref} className="underline-offset-2 active:underline">{replyingTo.sender_display_name}</Link> : replyingTo.sender_display_name}</p><p className="truncate type-caption font-semibold text-neutral-500">{messageReplyLabel(replyingTo)}</p></div><button type="button" aria-label={tx("Cancelar respuesta")} onClick={() => setReplyingTo(null)} className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-neutral-100 text-center type-small font-black text-neutral-500">×</button></div> : null}
        <MatchChatComposer body={body} composerRef={composerRef} onSubmit={send} onBodyChange={(value, element) => { setBody(value); resizeMatchChatComposer(element); updateMentionQuery(value, element.selectionStart ?? value.length) }} onTextareaClick={(event) => updateMentionQuery(event.currentTarget.value, event.currentTarget.selectionStart ?? event.currentTarget.value.length)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); if (mentionSuggestions[0]) insertMention(mentionSuggestions[0]); else event.currentTarget.form?.requestSubmit() } }} hasTopAttachment={Boolean(replyingTo)} leadingAction={!matchFinished ? <button type="button" data-tour="chat-proposals" aria-label={tx("Adjuntar propuesta")} title={tx("Proponer fecha o ubicación")} onClick={toggleProposalMenu} className={`inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-full transition active:scale-95 ${actionMode ? "bg-neutral-950 text-white" : "bg-neutral-100 text-neutral-700"}`}><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5"><path d="m20.5 11.5-8.8 8.8a6 6 0 0 1-8.5-8.5l9.2-9.2a4 4 0 0 1 5.7 5.7l-9.2 9.2a2 2 0 0 1-2.8-2.8l8.5-8.5" /></svg></button> : null} />
      </>}
    </MatchChatFrame>
  )
}
