...
export default function MatchChatPage() {
  ...
  async function loadFromServer() {
    ...
    setMessages(mergePendingMessages(snapshot.messages, pendingMessagesRef.current.values()))
  ...
  async function post(kind: ChatKind, payload: unknown = {}, text = "", refocusComposer = false) {
    ...
    const source = record(payload), previousBody = body, previousDateDraft = dateDraft, previousDateOptions = dateOptions, previousLocationDraft = locationDraft, clientId = createOptimisticClientId()
    const optimisticPayload = kind === "date_proposal" ? { clientId, options: (Array.isArray(source.options) ? source.options : []).map((startsAt, index) => ({ key: `date-${index + 1}`, startsAt: String(startsAt) })) } : kind === "location_proposal" ? { clientId, key: "location", name: String(source.name ?? ""), locationId: typeof source.locationId === "string" ? source.locationId : null } : { clientId }
    ...
    pendingMessagesRef.current.set(optimisticId, optimisticMessage); setMessages((current) => [...current, optimisticMessage]); setError(""); setMentionQuery(null); setActionMode(null)
    if (kind === "text") { setBody(""); if (composerRef.current) composerRef.current.style.height = "auto"; if (refocusComposer) focusComposerAfterSend() }
    ...
    else { setMessages((current) => current.filter((message) => message.id !== optimisticId)); if (kind === "text") setBody((current) => current || previousBody); if (kind === "date_proposal") { setDateDraft(previousDateDraft); setDateOptions(previousDateOptions); setActionMode("date") }; if (kind === "location_proposal") { setLocationDraft(previousLocationDraft); setActionMode("location") }; if (data?.error === "match_chat_read_only") setReadOnly(true); setError(data?.error === "match_chat_rate_limited" ? "Demasiados mensajes seguidos. Espera unos segundos." : data?.error === "match_chat_read_only" ? "El partido ya tiene resultado. El chat queda disponible solo para lectura." : chatErrorMessage(data?.error)) }
    return response.ok
  }

  function send(event: React.FormEvent) { event.preventDefault(); const text = body.trim(); if (text) void post("text", {}, text, true) }
  ...
  async function respond(messageId: string, optionKey: string, responseValue: "available" | "unavailable") {
    ...
    setMessages((current) => current.map((message) => message.id === messageId ? { ...message, responses: [...message.responses.filter((item) => !(item.userId === me && item.optionKey === optionKey)), optimisticResponse] } : message)); setError("")
  ...
  function proposalControls(message: Message, optionKey: string) {
    ...
    if (readOnly) return <div className="shrink-0 type-caption font-black text-neutral-500">✓ {yes} · ✕ {no}</div>
    return <div className="flex shrink-0 items-center gap-1.5"><button type="button" onClick={() => void respond(message.id, optionKey, "available")} aria-label={`Me viene bien · ${yes} votos`} title="Me viene bien" className={`inline-flex h-10 min-w-10 items-center justify-center gap-1 rounded-full border px-2 transition active:scale-95 ${mine === "available" ? "border-emerald-600 bg-emerald-600 text-white" : "border-neutral-200 bg-white text-neutral-500"}`}><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5"><path d="m5 12 4 4L19 6" /></svg><span className="type-caption font-black">{yes}</span></button><button type="button" onClick={() => void respond(message.id, optionKey, "unavailable")} aria-label={`No puedo · ${no} votos`} title="No puedo" className={`inline-flex h-10 min-w-10 items-center justify-center gap-1 rounded-full border px-2 transition active:scale-95 ${mine === "unavailable" ? "border-red-600 bg-red-600 text-white" : "border-neutral-200 bg-white text-neutral-500"}`}><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" className="h-5 w-5"><path d="m6 6 12 12M18 6 6 18" /></svg><span className="type-caption font-black">{no}</span></button></div>
  }
  ...
  return (
    <div ref={viewportRef} className="fixed inset-x-0 top-0 z-[60] mx-auto flex w-full max-w-md min-h-0 flex-col overflow-hidden bg-stone-50" style={{ height: "100dvh" }}>
      ...
      <div data-tour="chat-messages" className="mx-3 mt-2 flex min-h-0 flex-1 flex-col overflow-hidden rounded-2xl bg-neutral-100 shadow-inner" style={{ visibility: initialLoadComplete ? "visible" : "hidden" }}>{reservationSummary ? <div className="flex shrink-0 items-center gap-1.5 border-b border-blue-100 bg-blue-50 px-3 py-1.5 text-blue-900"><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-3.5 w-3.5 shrink-0"><path d="M12 17v5" /><path d="m5 3 14 0" /><path d="m7 3 1.5 8-3 3h13l-3-3L17 3" /></svg><p className="min-w-0 flex-1 truncate type-caption font-black" title={`${proposalDate(reservationSummary.scheduledAt)} · ${reservationSummary.locationText}`}><span className="uppercase tracking-wide text-blue-700">Reserva</span> · {proposalDate(reservationSummary.scheduledAt)} · {reservationSummary.locationText}</p></div> : null}<div ref={messagesRef} className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-3">
        ...
        <div className="space-y-2">
          {messages.map((message, index) => {
            const mine = message.sender_user_id === me || message.id.startsWith("optimistic-"), payload = record(message.payload), systemMessage = message.kind === "text" && Boolean(payload.systemType), previous = messages[index - 1], next = messages[index + 1]
            const previousSameSender = !systemMessage && previous && !(previous.kind === "text" && record(previous.payload).systemType) && previous.sender_user_id === message.sender_user_id, nextSameSender = !systemMessage && next && !(next.kind === "text" && record(next.payload).systemType) && next.sender_user_id === message.sender_user_id
            const sender = participants.find((item) => item.userId === message.sender_user_id), senderColor = participantColorClass(message.sender_user_id, participants), avatarSlot = !mine ? <span className="flex h-7 w-7 shrink-0 items-end">{!nextSameSender ? <PlayerAvatar player={{ displayName: sender?.displayName ?? message.sender_display_name, avatarUrl: sender?.avatarUrl ?? null }} size="sm" /> : null}</span> : null
            if (message.kind === "text" && payload.systemType === "reservation_confirmed") {
              ...
              return <div key={message.id} className="flex justify-center"><div className="w-[94%] rounded-2xl border border-emerald-200 bg-emerald-50 p-3 text-neutral-950 shadow-sm"><p className="type-caption font-black uppercase tracking-[0.14em] text-emerald-700">Partido programado</p><p className="mt-1 text-sm font-black">{scheduledAt ? proposalDate(scheduledAt) : String(payload.dateLabel ?? "")}</p><p className="mt-0.5 text-xs font-bold text-neutral-600">{locationText ?? "Pista reservada"}</p>{match && scheduledAt ? <div className="mt-2"><AddToCalendarButton leagueName={activeLeague.name} seasonName={activeSeason.name} round={matchRound ?? match.round} teamA={match.teamA} teamB={match.teamB} players={players} scheduledAt={scheduledAt} location={typeof payload.location === "string" ? payload.location : locationText} /></div> : null}</div></div>
            }
            if (message.kind !== "text") return <div key={message.id} className={`flex items-end gap-1.5 ${mine ? "justify-end" : "justify-start"}`}>{avatarSlot}<div className="w-[88%] rounded-2xl border border-neutral-200 bg-white p-3 text-neutral-950 shadow-sm"><div className="flex items-start justify-between gap-3"><div className="min-w-0">{!mine && !previousSameSender ? <p className={`truncate type-caption font-black ${senderColor}`}>{message.sender_display_name}</p> : null}<p className="text-sm font-black">{message.kind === "date_proposal" ? "Propuesta de fecha" : "Propuesta de ubicación"}</p></div><span className="shrink-0 type-caption font-bold text-neutral-400">{new Date(message.created_at).toLocaleTimeString("es-ES", { hour: "2-digit", minute: "2-digit" })}{mine ? <MessageReceipt message={message} me={me} participants={participants} /> : null}</span></div>{message.kind === "date_proposal" ? <div className="mt-2 space-y-2">{(Array.isArray(payload.options) ? payload.options : []).map((raw) => { const option = record(raw); const key = String(option.key ?? ""); const startsAt = String(option.startsAt ?? ""); return key && startsAt ? <div key={key} className="flex items-center gap-2 rounded-xl bg-neutral-50 px-2.5 py-2"><p className="min-w-0 flex-1 text-sm font-bold">{proposalVoteDate(startsAt)}</p>{proposalControls(message, key)}</div> : null })}</div> : <div className="mt-2 flex items-center gap-2 rounded-xl bg-neutral-50 px-2.5 py-2"><p className="min-w-0 flex-1 text-sm font-bold">{String(payload.name ?? "Ubicación propuesta")}</p>{proposalControls(message, String(payload.key ?? "location"))}</div>}</div></div>
            return <div key={message.id} className={`flex items-end gap-1.5 ${mine ? "justify-end" : "justify-start"}`}>{avatarSlot}<div className={`max-w-[80%] rounded-2xl px-3 py-2 shadow-sm ${mine ? "rounded-br-md bg-neutral-950 text-white" : "rounded-bl-md border border-neutral-200 bg-white text-neutral-950"}`}>{!mine && !previousSameSender ? <p className={`max-w-full truncate whitespace-nowrap type-caption font-black ${senderColor}`}>{message.sender_display_name}</p> : null}<p className="whitespace-pre-wrap break-words text-sm leading-5"><MentionText body={message.body} participants={participants} /></p><p className={`mt-0.5 text-right leading-none ${mine ? "text-neutral-300" : "text-neutral-400"}`}><span className="inline-block origin-right scale-90 type-caption">{new Date(message.created_at).toLocaleTimeString("es-ES", { hour: "2-digit", minute: "2-digit" })}{mine ? <MessageReceipt message={message} me={me} participants={participants} /> : null}</span></p></div></div>
          })}
        </div>
      </div></div>
      ...
      {readOnly ? <div className="mt-2 shrink-0 border-t border-neutral-200 bg-white px-3 pt-2 text-center type-caption font-bold text-neutral-500" style={{ paddingBottom: "max(10px, env(safe-area-inset-bottom, 0px))" }}>Partido finalizado · Chat en modo lectura</div> : <>
        {displayedCoordination?.status === "awaiting_booking" ? <div className="mx-3 mt-2 shrink-0 rounded-2xl border border-emerald-200 bg-emerald-50 p-2.5"><div className="flex items-center justify-between gap-2"><div className="min-w-0"><p className="text-sm font-black text-neutral-950">Pendiente de reserva</p><p className="type-caption font-semibold text-neutral-600">{displayedCoordination.approvedDates.length} horario{displayedCoordination.approvedDates.length === 1 ? "" : "s"} · {displayedCoordination.approvedLocations.length} ubicacion{displayedCoordination.approvedLocations.length === 1 ? "" : "es"} acordada{displayedCoordination.approvedLocations.length === 1 ? "" : "s"} 4/4</p></div></div><div className="mt-2"><MatchReservationConfirmation matchId={id} coordination={displayedCoordination} locations={activeLeague.locations} onConfirmed={loadFromServer} /></div></div> : null}
        {displayedCoordination?.status === "coordinating" ? <div className="mx-3 mt-2 shrink-0 rounded-xl bg-neutral-100 px-2.5 py-1.5 text-center type-caption font-black uppercase tracking-wide text-neutral-500">Coordinando</div> : null}
        ...
        <form data-tour="chat-composer" onSubmit={send} className="mt-2 flex shrink-0 items-end gap-2 border-t border-neutral-200 bg-white px-3 pt-2 shadow-[0_-4px_16px_rgba(0,0,0,0.05)]" style={{ paddingBottom: "max(8px, var(--match-chat-bottom-inset, env(safe-area-inset-bottom, 0px)))" }}>
