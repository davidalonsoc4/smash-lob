...
export default function MatchChatPage() {
  ...
  function beginReplySwipe(event: React.PointerEvent<HTMLDivElement>, message: Message, messageKey: string) {
    if (readOnly || message.id.startsWith("optimistic-") || (message.kind === "text" && Boolean(record(message.payload).systemType))) return
        ...
        <div>
          {messages.map((message, index) => {
            ...
            if (message.kind !== "text") {
              ...
              const locationKey = String(payload.key ?? "location"), locationDetailKey = `${message.id}:${locationKey}`, locationExpanded = proposalExpanded || expandedProposalOptionKey === locationDetailKey
              const senderHref = participantProfileHref(sender)
              return <div key={messageKey} {...replyGestureProps} className={`flex items-start gap-1 ${mine ? "justify-end" : "justify-start"} ${rowSpacing} ${enterClass}`}>{avatarSlot}<div className={`w-[90%] rounded-2xl border border-neutral-200 bg-white p-2.5 text-neutral-950 shadow-sm ${!previousSameSender ? mine ? "rounded-tr-md" : "rounded-tl-md" : ""}`}>{!mine && !previousSameSender ? senderHref ? <Link href={senderHref} onClick={(event) => event.stopPropagation()} className={`block w-fit max-w-full truncate type-caption font-black underline-offset-2 active:underline ${senderColor}`}>{message.sender_display_name}</Link> : <p className={`truncate type-caption font-black ${senderColor}`}>{message.sender_display_name}</p> : null}<button type="button" aria-expanded={proposalExpanded} onClick={(event) => { event.stopPropagation(); setExpandedProposalOptionKey(null); setExpandedProposalId((current) => current === message.id ? null : message.id) }} className="flex w-full items-start justify-between gap-3 text-left"><div className="min-w-0"><p className="text-sm font-black">{message.kind === "date_proposal" ? "Propuesta de fecha" : "Propuesta de ubicación"}</p><p className="type-caption font-semibold text-neutral-400">Toca para ver quién ha votado</p></div><span className="shrink-0 type-caption font-bold text-neutral-400">{new Date(message.created_at).toLocaleTimeString("es-ES", { hour: "2-digit", minute: "2-digit" })}{mine ? <MessageReceipt message={message} me={me} participants={participants} /> : null}</span></button>{message.kind === "date_proposal" ? <div className="mt-1.5 space-y-1.5">{(Array.isArray(payload.options) ? payload.options : []).map((raw) => { const option = record(raw); const key = String(option.key ?? ""); const startsAt = String(option.startsAt ?? ""); const detailKey = `${message.id}:${key}`, optionExpanded = proposalExpanded || expandedProposalOptionKey === detailKey; return key && startsAt ? <div key={key} className="flex items-start gap-2 rounded-xl bg-neutral-50 px-2.5 py-1.5"><div className="min-w-0 flex-1"><button type="button" aria-expanded={optionExpanded} onClick={(event) => { event.stopPropagation(); setExpandedProposalOptionKey((current) => current === detailKey ? null : detailKey) }} className="block w-full text-left"><span className="block text-sm font-bold">{proposalVoteDate(startsAt)}</span>{optionHasAgreement(message, key) ? <span className="mt-0.5 inline-flex rounded-full bg-emerald-100 px-1.5 py-px type-caption font-black uppercase tracking-wide text-emerald-700">Acuerdo 4/4</span> : null}</button>{optionExpanded ? proposalVoteDetailRows(message, key) : null}</div>{proposalControls(message, key)}</div> : null })}</div> : <div className="mt-1.5 flex items-start gap-2 rounded-xl bg-neutral-50 px-2.5 py-1.5"><div className="min-w-0 flex-1"><button type="button" aria-expanded={locationExpanded} onClick={(event) => { event.stopPropagation(); setExpandedProposalOptionKey((current) => current === locationDetailKey ? null : locationDetailKey) }} className="block w-full text-left"><span className="block text-sm font-bold">{String(payload.name ?? "Ubicación propuesta")}</span>{optionHasAgreement(message, locationKey) ? <span className="mt-0.5 inline-flex rounded-full bg-emerald-100 px-1.5 py-px type-caption font-black uppercase tracking-wide text-emerald-700">Acuerdo 4/4</span> : null}</button>{locationExpanded ? proposalVoteDetailRows(message, locationKey) : null}</div>{proposalControls(message, locationKey)}</div>}</div></div>
            }
            const senderHref = participantProfileHref(sender)
