...
function chatParticipantsLabel(chat: ChatItem) { const rivals = chat.rivals.filter(Boolean); const rivalLabel = rivals.length > 1 ? `${rivals.slice(0, -1).join(", ")} y ${rivals.at(-1)}` : rivals[0] ?? "Rivales"; return `Con ${chat.partner || "Pareja"} vs ${rivalLabel}` }
...
export default function ChatsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setChats(Array.isArray(data?.chats) ? data.chats : []); setError(false); setLoading(false)
  }, [activeLeague.id, activeSeason.id])
  useEffect(() => { const initialTimer = window.setTimeout(() => { void load() }, 0); const timer = window.setInterval(() => { if (!document.hidden) void load() }, 10_000); return () => { window.clearTimeout(initialTimer); window.clearInterval(timer) } }, [load])
  ...
  return <div className="compact-page space-y-3">
    ...
    {activeChats.length > 0 ? <section className="space-y-2"><h2 className="type-section-title text-lg font-black">Chats activos</h2><ChatCards chats={activeChats} /></section> : null}
