...
begin
  ...
  set invite_code = upper(trim(p_code))
  where id = p_league_id
  returning id, invite_code into league_id, invite_code;
...
create or replace function public.server_delete_league(
  p_league_id uuid
)
returns void
language plpgsql
...
revoke all on function public.server_delete_league(uuid) from public;
revoke all on function public.server_delete_league(uuid) from anon;
revoke all on function public.server_delete_league(uuid) from authenticated;
grant execute on function public.server_delete_league(uuid) to service_role;
