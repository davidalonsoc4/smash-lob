...
where invite.league_id = league.id
  ...
  and upper(trim(invite.code)) <> upper(trim(league.invite_code));
...
begin
  update public.invites as previous_invite
  set revoked_at = now()
  where previous_invite.league_id = p_league_id
    and previous_invite.revoked_at is null;
