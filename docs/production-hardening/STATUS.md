...
- v1.14.40 queda implementada y validada localmente en `codex/anonymous-spectator-access`: `npm run release:check` supera 200 archivos / 757 pruebas unitarias e integración, 66 E2E, build de 1.055.669 bytes gzip y auditoría runtime con cero vulnerabilidades; `git diff --check` limpio. Sin migraciones. No se ha publicado esta funcionalidad en PRE.
...
- v1.14.40 en curso en `codex/anonymous-spectator-access`: la invitación y el QR ofrecen una vista pública sin sesión con una API de solo lectura que devuelve únicamente datos de liga, temporada, marcador, clasificación y calendario. Chats, actividad, pagos, incidencias e identidad de cuenta quedan fuera del payload; se preservan la fase secreta y el calendario progresivo. No requiere migración. Validación focalizada inicial: 2 tests unitarios de sanitización, 18 E2E públicos en móvil/escritorio, typecheck, i18n, inventario API y coherencia de versión superados; falta la puerta completa final.
