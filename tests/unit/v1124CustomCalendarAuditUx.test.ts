...
describe("v1.12.4 custom calendar audit and creation UX", () => {
  ...
  it("keeps audit and reroll visible for every valid custom duration and separates creation decisions into independent cards", () => {
    ...
    const calendarPanelSource = fs.readFileSync(
      path.join(process.cwd(), "src/components/admin/season/BalancedCalendarAuditPanel.tsx"),
      "utf8",
    )
    const newSeasonFormSource = fs.readFileSync(
    ...
    expect(calendarPanelSource).toContain("onClick={rerollCalendar}")
    expect(newSeasonFormSource).toContain("data-season-calendar-type")
    expect(newSeasonFormSource).toContain("data-season-duration")
    expect(newSeasonFormSource).toContain("data-season-calendar-visibility")
  })
})
