...
describe("v1.8.0 date and player-name contracts", () => {
  ...
  it("includes year in every visible day/month formatter", async () => { const misses: string[] = []; for (const file of await walk("src")) { const source = await readFile(file, "utf8"); for (const pattern of [/(?:new Intl\.DateTimeFormat\([^,]+,\s*|toLocaleDateString\([^,]+,\s*)\{([\s\S]*?)\}\)/g, /toLocaleString\([^,]+,\s*\{([\s\S]*?)\}\)/g]) for (const match of source.matchAll(pattern)) { const body = match[1]; if (body.includes("day:") && body.includes("month:") && !body.includes("year:")) misses.push(file.replaceAll("\\", "/")) } } expect([...new Set(misses)], misses.join("\n")).toEqual([]) })
})
