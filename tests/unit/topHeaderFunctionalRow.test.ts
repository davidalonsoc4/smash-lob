...
describe("top functional header row", () => {
  it("reserves vertical space instead of shrinking page headings", async () => {
    ...
    expect(shell).toContain("function getFloatingTop()")
    expect(shell).toContain('return "max(10px, calc(var(--app-safe-top) + 8px))"')
    expect(shell).toContain("data-floating-top-toolbar")
    expect(shell).toContain('className={`flex items-center gap-2')
    expect(shell).toContain("top: getFloatingTop()")
    ...
    expect(shell).toContain("{hasSpectatorShareControl ? <SpectatorFloatingControls /> : null}")
    for (const control of [invite, help, spectatorShare]) {
      expect(control).toContain("shrink-0")
      expect(control).not.toContain("getFloatingTop")
      expect(control).not.toContain('position: "fixed"')
