...
describe("v1.10.0 settings search coverage", () => {
  it("finds every new season/admin setting from its natural Spanish terms", () => {
    ...
    expect(searchSettingsEntries(entries, "modo de experiencia", "es")[0]?.id).toBe("adminView")
    expect(searchSettingsEntries(entries, "fase secreta", "es")[0]?.id).toBe("preseasonSecrets")
    expect(searchSettingsEntries(entries, "jornada de apertura", "es")[0]?.id).toBe("openingRound")
    expect(searchSettingsEntries(entries, "ubicación apertura", "es")[0]?.id).toBe("openingRound")
    expect(searchSettingsEntries(entries, "bolas asignadas por la organización", "es")[0]?.id).toBe("organizationBalls")
    expect(searchSettingsEntries(entries, "custodio de bolas", "es")[0]?.id).toBe("organizationBalls")
    expect(entries.find((entry) => entry.id === "organizationBalls")?.href).toBe("/admin/season#bolas-organizacion")
    expect(searchSettingsEntries(entries, "calendario progresivo", "es")[0]?.id).toBe("calendarVisibility")
    expect(searchSettingsEntries(entries, "desbloqueo progresivo", "es")[0]?.id).toBe("calendarVisibility")
    expect(searchSettingsEntries(entries, "reroll", "es")[0]?.id).toBe("rerollCalendar")
  ...
  it("keeps the new searchable settings translated in all supported locales", () => {
    for (const locale of ["es", "en", "eu"] as const) {
      ...
      expect(entries.some((entry) => entry.id === "adminView")).toBe(true)
      expect(entries.some((entry) => entry.id === "preseasonSecrets")).toBe(true)
      expect(entries.some((entry) => entry.id === "openingRound")).toBe(true)
      expect(entries.some((entry) => entry.id === "organizationBalls")).toBe(true)
      expect(entries.some((entry) => entry.id === "calendarVisibility")).toBe(true)
      expect(entries.some((entry) => entry.id === "rerollCalendar")).toBe(true)
