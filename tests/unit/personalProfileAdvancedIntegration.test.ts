...
describe("personal profile advanced statistics", () => {
  it("exposes summary, relation rankings and head-to-head under the existing filters", async () => {
    ...
    expect(view).toContain("Abrir cara a cara")
    expect(view).toContain('countText(row.losses, "derrota", "derrotas")')
    expect(page).toContain('setSection("head-to-head")')
    expect(page).toContain("onRelationSelect={openHeadToHead}")
  ...
  it("uses stable participant identities and requests avatars only for the profile history", async () => {
    ...
    expect(types).toContain("profilePlayerId?: string | null")
    expect(types).toContain("profileLeagueId?: string | null")
    expect(server).toContain("sourcePlayerLeagueById")
    expect(server).toContain("profileLeagueId: match.leagueId")
    expect(page).toContain("activateLeague(sourceLeagueId)")
    expect(page).toContain('router.push(`/player/${playerId}`)')
  })
})
