...
describe("Media Kit Welcome Pack", () => {
  ...
  it("uses the real app icon and common Media Kit branding", () => {
    ...
    expect(html).toContain(".creator-row { margin-top: 3.2mm; display: flex; align-items: center; justify-content: center; gap: .95mm; }")
    expect(html).toContain(".creator-icon { width: 3mm; height: 3mm;")
    expect(html).toContain(".creator-name { margin-top: .3mm; color: #f4f1ea; font-size: 1.2mm;")
  ...
  it("keeps the final overgrip measurements explicit", () => {
    ...
    expect(WELCOME_PACK_OVERGRIP_BAND.widthMm).toBe(110)
    expect(WELCOME_PACK_OVERGRIP_BAND.heightMm).toBe(18)
    expect(WELCOME_PACK_OVERGRIP_BAND.sideReserveMm).toBe(10)
    expect(WELCOME_PACK_OVERGRIP_BAND.contentWidthMm).toBe(90)
  })
