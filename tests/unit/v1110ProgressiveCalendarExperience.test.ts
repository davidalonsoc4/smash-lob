...
describe("v1.11.0 progressive calendar and player experience", () => {
  ...
  it("protects hidden pairings across chat, activity and push while supporting a true balanced-calendar reroll", async () => {
    ...
    expect(serverActivity).toContain("hiddenEventIds")
    expect(serverActivity).toContain("isTargetedCustodianActivityVisibleToPlayer")
    ...
    expect(mutations).toContain("season_calendar_reroll_has_results")
    expect(mutations).toContain('"reroll_season_calendar_matches"')
    expect(mutations).toContain('if (!reroll && season.status !== "upcoming")')
    ...
    expect(repairApi).toContain("{ requireMutable: !reroll }")
    expect(admin).toContain("t.adminSeason.rerollButton")
    expect(admin).toContain("rerollCalendar")
    expect(admin).toContain("const canReroll = !hasRecordedResults")
    expect(admin).toContain("t.adminSeason.rerollBlockedByResults")
  })
})
