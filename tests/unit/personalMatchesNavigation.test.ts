...
describe("personal matches navigation", () => {
  it("uses a compact four-destination nav with friendly chats and keeps Back only on subpages", async () => {
    ...
    expect(nav.indexOf('href: "/personal-matches"')).toBeLessThan(
      nav.indexOf('href: "/personal-matches/chats"'),
    )
    expect(nav.indexOf('href: "/personal-matches/chats"')).toBeLessThan(
