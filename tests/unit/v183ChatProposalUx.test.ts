...
describe("v1.8.3 chat proposal UX", () => {
  ...
  it("shows weekday-aware proposal dates and large visual voting controls", async () => {
    ...
    expect(page).toContain('aria-pressed={mine === "unavailable"}')
    expect(page).toContain('Quitar mi voto favorable')
    expect(page).toContain('Quitar mi voto negativo')
    expect(page).toContain('· ${yes} votos')
    expect(page).toContain('· ${no} votos')
  })
