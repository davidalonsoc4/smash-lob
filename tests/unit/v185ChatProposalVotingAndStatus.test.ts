...
describe("v1.8.5 chat proposal voting and programming status", () => {
  it("uses weekday-aware dates in proposal draft and sent proposal rows", async () => { const page = await readFile("src/app/match/[id]/chat/page.tsx", "utf8"); expect(page).toContain("calendarWeekday(localDateValue(date), locale)"); expect(page).toContain("proposalVoteDate(startsAt, locale)"); expect(page).toContain('weekday: "long"'); expect(page).toContain('year: "numeric"') })
